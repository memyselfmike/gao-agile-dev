...
@pytest.fixture
def brian_orchestrator(mock_workflow_registry):
...
class TestScaleLevelAssessment:
    ...
    @pytest.mark.asyncio
    async def test_level_0_bug_fix(self, brian_orchestrator):
        """Test Level 0 assessment for simple bug fix."""
        prompt = "Fix the login bug where users can't sign in"

        # Mock the Anthropic API response
        mock_response = Mock()
        mock_response.content = [Mock(text='{"scale_level": 0, "project_type": "bug_fix", "is_greenfield": false, "is_brownfield": true, "is_game_project": false, "estimated_stories": 1, "estimated_epics": 1, "technical_complexity": "low", "domain_complexity": "low", "timeline_hint": "hours", "confidence": 0.9, "reasoning": "Single bug fix requiring 1 story", "needs_clarification": false, "clarifying_questions": []}')]

        with patch.object(brian_orchestrator.anthropic.messages, 'create', return_value=mock_response):
    ...
    @pytest.mark.asyncio
    async def test_level_2_medium_project(self, brian_orchestrator):
    ...
    @pytest.mark.asyncio
    async def test_level_3_large_project(self, brian_orchestrator):
        ...
        mock_response = Mock()
        mock_response.content = [Mock(text='{"scale_level": 3, "project_type": "greenfield", "is_greenfield": true, "is_brownfield": false, "is_game_project": false, "estimated_stories": 25, "estimated_epics": 3, "technical_complexity": "high", "domain_complexity": "medium", "timeline_hint": "months", "confidence": 0.85, "reasoning": "Large project requiring architecture and 25 stories across 3 epics", "needs_clarification": false, "clarifying_questions": []}')]
    ...
    @pytest.mark.asyncio
    async def test_level_4_enterprise(self, brian_orchestrator):
...
class TestProjectTypeRouting:
    ...
    @pytest.mark.asyncio
    async def test_greenfield_routing(self, brian_orchestrator):
    ...
    @pytest.mark.asyncio
    async def test_game_project_routing(self, brian_orchestrator):
...
class TestClarificationQuestions:
    ...
    @pytest.mark.asyncio
    async def test_ambiguous_prompt_returns_questions(self, brian_orchestrator):
        """Test that ambiguous prompts return clarifying questions."""
        prompt = "Fix it"

        mock_response = Mock()
        mock_response.content = [Mock(text='{"scale_level": 0, "project_type": "bug_fix", "is_greenfield": false, "is_brownfield": true, "is_game_project": false, "estimated_stories": 1, "estimated_epics": 1, "technical_complexity": "low", "domain_complexity": "low", "timeline_hint": null, "confidence": 0.3, "reasoning": "Prompt too ambiguous", "needs_clarification": true, "clarifying_questions": ["What needs to be fixed?", "Is this a bug fix or feature request?"]}')]
...
class TestFallbackHandling:
    ...
    @pytest.mark.asyncio
    async def test_ai_failure_fallback(self, brian_orchestrator):
        """Test that AI failure results in conservative fallback."""
        prompt = "Build a todo app"

        # Simulate AI failure
        with patch.object(brian_orchestrator.anthropic.messages, 'create', side_effect=Exception("API Error")):
            workflow_sequence = await brian_orchestrator.assess_and_select_workflows(prompt)
...
class TestForceScaleLevel:
    ...
    @pytest.mark.asyncio
    async def test_force_scale_level_override(self, brian_orchestrator):
        """Test that force_scale_level overrides AI assessment."""
        prompt = "Fix a bug"  # Would normally be Level 0

        mock_response = Mock()
        mock_response.content = [Mock(text='{"scale_level": 0, "project_type": "bug_fix", "is_greenfield": false, "is_brownfield": true, "is_game_project": false, "estimated_stories": 1, "estimated_epics": 1, "technical_complexity": "low", "domain_complexity": "low", "timeline_hint": "hours", "confidence": 0.9, "reasoning": "Bug fix", "needs_clarification": false, "clarifying_questions": []}')]
